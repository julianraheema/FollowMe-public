...
void app_init(int argc, char *argv[])
{
  ...
  enum sl_rtl_error_code ec = followme_distance_init();
  app_assert(ec == SL_RTL_ERROR_SUCCESS, 
            "Failed to initialize distance parameters." APP_LOG_NL);
...
void aoa_cte_on_iq_report(aoa_db_entry_t *tag, aoa_iq_report_t *iq_report)
{
  ...
  switch(report_mode){
    ...
    case (ANGLE_REPORT):
      ...
      //AoA calculation via RTL library, distance calculation via followme_distance.c
    ...
    default:
      ...
      // Compile and send angle payload
      sc = aoa_serialize_angle(&angle, &payload);
      app_assert(sc == SL_STATUS_OK,
                "[E: 0x%04x] Failed to serialize the angle payload." APP_LOG_NL,
                (int)sc);
      ...
      //done in followme_positioning.c
      ec = followme_position((aoa_state_t *)tag->user_data,
                        iq_report,
                        &angle,
                        &position,
                        locator_id,
